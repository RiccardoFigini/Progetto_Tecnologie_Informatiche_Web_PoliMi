...
public class CreateAuction extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String minimumRise_s = request.getParameter("minimumRise");
		String startDate_s = request.getParameter("startDate");
		String startTime_s = request.getParameter("startTime")+":00";
		String endDate_s = request.getParameter("endDate");
		String endTime_s = request.getParameter("endTime")+":00";
		...
		try {
			...
			for (int item : selectedArticlesCodes) {
				...
				if ((availableArticles.stream().map(x -> x.getCode()).collect(Collectors.toList()).contains(item) == false) ||
